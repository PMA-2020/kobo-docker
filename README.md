...
1. Run `make get-ips`
2. Set the `HOST_ADDRESS` variable of `envfile.local.txt` to the IP that shows as a result of step (1). If multiple IP addresses show up, try the first one and continue with the rest of this setup. If you get to the end and get error code 400 in the browser, repeat this whole process, trying the other IPs shown one at a time.
3. Run `make setup`
4. A lot of tasks will run. This could take 10-30 minutes. After waiting quite awhile, once the text `enketo_express_1  | 20:49:19 0|enketo   | Worker 2 ready for duty at port 8005! (environment: production)` appears, try accessing `http://${HOST_ADDRESS}:${KPI_PUBLIC_PORT}` (substituting in the values entered in [`envfile.local.txt`](./envfile.local.txt)).
